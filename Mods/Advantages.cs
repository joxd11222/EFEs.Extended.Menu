    ...
    public static class Advantages
    {
        ...
        public static void TagSelf()
        {
            ...
            else
            {
                ...
                {
                    VRRig rig = VRRigCache.Instance.GetAllRigs()
        ...
        {
            foreach (var vrrig in VRRigCache.Instance.GetAllRigs().Where(vrrig => VRRig.LocalRig.IsTagged() && !vrrig.IsTagged() && !GTPlayer.Instance.disableMovement && Vector3.Distance(vrrig.headMesh.transform.position, GorillaTagger.Instance.bodyCollider.transform.position) < tagAuraDistance))
                ReportTag(vrrig);
        }
        ...
        {
            foreach (var vrrig in from vrrig in VRRigCache.Instance.GetAllRigs() let distance = Vector3.Distance(vrrig.headMesh.transform.position, giving.transform.position) where giving.IsTagged() && !vrrig.IsTagged() && !GTPlayer.Instance.disableMovement && distance < tagAuraDistance && !VRRig.LocalRig.IsLocal() && VRRig.LocalRig.IsTagged() select vrrig)
        ...
        public static void TagAll()
        {
            ...
            else
            {
                ...
                {
                    bool isInfectedPlayers = VRRigCache.Instance.GetAllRigs().Any(vrrig => !vrrig.IsTagged());
                    if (isInfectedPlayers)
                    {
                        foreach (var vrrig in VRRigCache.Instance.GetAllRigs().Where(vrrig => !vrrig.IsTagged()))
                        {
                            VRRig.LocalRig.enabled = false;
