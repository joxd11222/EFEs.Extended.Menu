    ...
    public static class Safety
    {
        ...
        public static void AntiReport(Action<VRRig, Vector3> onReport)
        {
            ...
            foreach (GorillaPlayerScoreboardLine line in GorillaScoreboardTotalUpdater.allScoreboardLines)
            {
                ...
                foreach (var vrrig in from vrrig in VRRigCache.Instance.GetAllRigs() where !vrrig.isLocal where OverlappingButton(vrrig, report.position) || (antiMute && OverlappingButton(vrrig, line.muteButton.gameObject.transform.position)) where !smartAntiReport || SmartAntiReport(line.linePlayer) select vrrig)
        ...
        {
            foreach (var vrrig in VRRigCache.Instance.GetAllRigs().Where(vrrig => !vrrig.isOfflineVRRig && string.Join(",", vrrig._playerOwnedCosmetics).Contains("LBAAK") || string.Join(",", vrrig._playerOwnedCosmetics).Contains("LBAAD") || string.Join(",", vrrig._playerOwnedCosmetics).Contains("LMAPY")))
            {
                try
                {
        ...
        {
            foreach (var vrrig in VRRigCache.Instance.GetAllRigs().Where(vrrig => !vrrig.isOfflineVRRig && Visuals.specialCosmetics.Keys.Any(x => string.Join(",", vrrig._playerOwnedCosmetics).Contains(x))))
            {
                try
                {
        ...
        public static void CosmeticNotifications()
        {
            ...
            {
                foreach (var cosmetic in Visuals.specialCosmetics.Where(cosmetic => string.Join(",", rig._playerOwnedCosmetics).Contains(cosmetic.Key)))
                {
                    specialRig = rig;
        ...
        public static void NameSpoof()
        {
            ...
            {
                if (!VRRigCache.Instance.GetAllRigs().Contains(rig))
                    toRemove.Add(rig);
            }
        ...
        public static void ColorSpoof()
        {
            ...
            foreach (var rig in VRRigCache.Instance.GetAllRigs().Where(rig => !rig.isLocal).Where(rig => !colorSpoofRigs.Contains(rig)))
            {
                GorillaTagger.Instance.myVRRig.SendRPC("RPC_InitializeNoobMaterial", GetPlayerFromVRRig(rig), Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f));
